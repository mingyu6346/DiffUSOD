...
class GrayscaleMetricRecorderV1:
    ...
    def get_results(self, num_bits: int = 4, return_ndarray: bool = False) -> dict:
        """
        返回指标计算结果：
...
class GrayscaleMetricRecorderV2:
    ...
    suppoted_metrics = ["mae", "em", "sm", "fm"]

    def __init__(self, metric_names=("sm", "mae", "fmeasure", "em")):
        """
...
def metrics_v1():
    """
        ...
        使用方法:
                metrics = metrics_v1()
                for i, (image, gt, depth) in enumerate(train_loader):
                    pred = model(image, depth)
                    metrics.step(pred, mask)
                metrics_dict = metrics.show()
    :return:    获取评价指标的对象
    """
    return GrayscaleMetricRecorderV2(metric_names=GrayscaleMetricRecorderV2.suppoted_metrics)


def metrics_dict_to_float(validate_metrics_dict: dict):
    maxem = validate_metrics_dict['maxem']
    avgem = validate_metrics_dict['avgem']
    adpem = validate_metrics_dict['adpem']

    sm = validate_metrics_dict['sm']

    maxfm = validate_metrics_dict['maxfm']
    avgfm = validate_metrics_dict['avgfm']
    adpfm = validate_metrics_dict['adpfm']

    mae = validate_metrics_dict['mae']

    performance = round((1 - mae) + maxem + avgem + adpem + sm + maxfm + avgfm + adpfm, 4)
...
def get_metrics(pred_root, mask_root):
    ...
    metrics = metrics_v1()
    for mask, pred in tqdm(zip(masks, preds)):
    ...
    # avgfm = x_dict['avgfm']
    #
    # max_performance = (1 - mae) + maxem + sm + maxfm
    # avg_performance = (1 - mae) + avgem + sm + avgfm
    # print(f"max_performance: {max_performance:.4f} / 4.0000")
    # print(f"avg_performance: {avg_performance:.4f} / 4.0000")
    # print(x_dict)
    # print(metrics_v2.show())
    print(x_dict)
    print(metrics_dict_to_float(x_dict))


if __name__ == "__main__":
    mask_root = "/media/omnisky/jixie/zmy/project/Diffusion/results/matlab/ORSSD"
    pred_root = "/media/omnisky/jixie/zmy/project/Diffusion/results/matlab/ORSSD"
    get_metrics(pred_root, mask_root)

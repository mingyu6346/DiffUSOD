import os
from  utils.affiliated_utils import backup_empty_files
...
def get_loader(cfg):
    ...
    test_dataset_expand = SampleDataset(full_dataset=instantiate_from_config(cfg.test_dataset.ORSSD), interval=10)
    test_dataset = torch.utils.data.ConcatDataset([test_dataset, test_dataset_expand])
    test_dataset_expand = SampleDataset(full_dataset=instantiate_from_config(cfg.test_dataset.EORSSD), interval=30)
    test_dataset = torch.utils.data.ConcatDataset([test_dataset, test_dataset_expand])
...
if __name__ == "__main__":
    ...
    parser.add_argument('--results_folder', type=str, default='results/pth/SMT_relight_2_E', help='None for saving in wandb folder.')
    parser.add_argument('--num_epoch', type=int, default=300)
    parser.add_argument('--batch_size', type=int, default=16)
    parser.add_argument('--gradient_accumulate_every', type=int, default=1)
    parser.add_argument('--num_workers', type=int, default=8)
    parser.add_argument('--lr_min', type=float, default=5e-6)
    parser.add_argument('--project_name', type=str, default='SMT_relight')

...
from .validate_metrics import get_metrics
...
def eval(mask_path='./Dataset/TestDataset',
    ...
    for dataset in [dataset_name]:
        mask_root = os.path.join(mask_path, dataset,'test','gt')
        print(dataset, mask_root)
        # mask_root = os.path.join(mask_path, 'gt')
        pred_root = os.path.join(pred_path, dataset)
        mask_name_list = sorted(os.listdir(mask_root))

        # results = get_metrics(pred_root, mask_root)/

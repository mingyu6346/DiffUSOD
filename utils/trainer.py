...
from utils.validate_metrics import metrics_v1
...
class Trainer(object):
    def __init__(
        ...
        project_name = getattr(cfg, "project_name")
        self.accelerator.init_trackers(project_name, config=dict(cfg))
    ...
    def load(self, resume_path: str = None, pretrained_path: str = None):
        ...
        if resume_path is not None:
            ...
            data = torch.load(pretrained_path, map_location=device, weights_only=True)
        else:
    ...
    def train(self):
        ...
        for epoch in range(self.cur_epoch, self.train_num_epoch):
            ...
            with tqdm(total=len(self.train_loader), disable=not accelerator.is_main_process, ncols=100) as pbar:
                for data in self.train_loader:
                    ...
                    pbar.set_description(
                        f'Epoch:{epoch}/{self.train_num_epoch} lr: {self.opt.param_groups[0]["lr"]:.1e} loss: {loss_sm.avg:.4f}({loss_sm.global_avg:.4f})')
                    self.accelerator.log({'loss': loss_sm.avg, 'lr': self.opt.param_groups[0]['lr']}, step=epoch)
                    pbar.update()
            ...
            if (epoch + 1) % 1 == 0 or (epoch >= self.train_num_epoch * 0.7):
                ...
                accelerator.log({'mae': mae, 'best_mae': best_mae}, step=epoch)
                if mae == best_mae:
                    self.save("best")

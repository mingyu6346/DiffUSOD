...
from tifffile.tifffile import kwargs_notnone
...
class FinalPredUint(nn.Module):
    def __init__(self, T=5):
        super(self).__init__()
        self.conv = nn.Sequential(
            nn.Conv2d(T, T * 4, kernel_size=3, stride=1, padding=1, bias=False),
            nn.BatchNorm2d(T * 4),
            nn.GELU(),
            nn.Conv2d(T * 4, T, kernel_size=3, stride=1, padding=1, bias=False),
        )
...
class CondGaussianDiffusion(GaussianDiffusion):
    ...
    def get_params_flops(self, gpu=2, batchsize=1, get_fps=False):
        from utils.affiliated_utils import param, flops, fps, fps_diff
        param(self)
        flops(self, gpu=gpu, count=2, batchsize=batchsize)
        # fps(self, epoch_num=10, size=384, gpu=gpu, count=2) if get_fps else None
        fps_diff(self, epoch_num=5, size=384, gpu=gpu) if get_fps else None
...
if __name__ == '__main__':
    ...
    model = net(class_num=1, mask_chans=1).eval()
    diff_model = CondGaussianDiffusion(model=model, image_size=384, channels=1, cond_channels=3, noise_d=64,
                                       num_sample_steps=10, pred_objective='x0',
                                       loss_type='model.loss.structure_loss').eval().cuda()

    train_size = 384

    cond_image = torch.randn(1, 3, train_size, train_size).cuda()
    noise = torch.randn(1, 1, train_size, train_size).cuda()

    # Train
    y = diff_model(noise, cond_image)
    print(y)  # loss

    # Sample
    # y = diff_model.sample(cond_image)

    # diff_model.get_params_flops()
